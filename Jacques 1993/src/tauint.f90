...
CONTAINS
   ...
   recursive subroutine tauint2(xmax,ymax,zmax,xcell &
            ,ycell,zcell,tflag,iseed,delta,sflag,weight,flag)

   use constants,   only : PI,nxg,nyg,nzg,OFFSET
   ...
   use opt_prop,    only : wave,kappa,n1,n2
   use noisey_mod

   implicit none

   integer iseed,xcell,ycell,zcell,flag
   real xmax,ymax,zmax,weight
   real ran2
   logical tflag,sflag
...
!***** calculate smax -- maximum distance photon can travel
   ...
   if(smax.lt.delta) then
      call inf_bound(xcur,ycur,zcur,xmax,ymax,zmax,xcell,ycell,zcell,d,delta &
                 ,tflag,sflag,weight,iseed)
!      tflag=.TRUE.
      return
      print*,'here'
   endif
...
!*****************************************************
   ...
   subroutine inf_bound(xcur,ycur,zcur,xmax,ymax,zmax,xcell,ycell,zcell,d,delta &
                       ,tflag,sflag,weight,iseed)
   
      use constants, only : nxg,nyg,nzg,OFFSET
      use photon_vars, only : xp,yp,zp,nxp,nyp,nzp
   
      implicit none
      
      real :: xcur,ycur,zcur,xmax,ymax,zmax,d,delta,weight
      integer :: xcell,ycell,zcell,iseed
      logical :: tflag,sflag
      
         if(xcur.gt.(2.*xmax)-OFFSET)then
         xcur=0.+OFFSET
         xp=xcur-xmax
         yp=ycur-ymax
         zp=zcur-zmax
...
!                      tflag,iseed,delta,sflag,weight,0)
      else if(xcur.lt.OFFSET)then
         xcur=2.*xmax-OFFSET
         xp = xcur-xmax
         yp=ycur-ymax
         zp=zcur-zmax
...
!         call tauint2(xmax,ymax,zmax,xcell,ycell,zcell,&
!                      tflag,iseed,delta,sflag,weight,0)
      else if(ycur.gt.(2.*ymax)-OFFSET)then
         ycur=0.+OFFSET
         yp=ycur-ymax
         xp=xcur-xmax
         zp=zcur-zmax
...
!         call tauint2(xmax,ymax,zmax,xcell,ycell,zcell,&
!                      tflag,iseed,delta,sflag,weight,0)
       else if(zcur.gt.(2.*zmax)-OFFSET)then !.or.zcur.lt.0.0001
            xp=xp+d*nxp
            yp=yp+d*nyp
            zp=zmax-OFFSET
            xcell=int(nxg*(xp+xmax)/(2.*xmax))+1
            ycell=int(nyg*(yp+ymax)/(2.*ymax))+1
            zcell=int(nzg*(zp+zmax)/(2.*zmax))+1
   call fresnel(sflag,tflag,iseed,weight,xcur,ycur &
               ,xmax,ymax,zmax,delta,xcell,ycell,zcell)
      else

         if(zcur.gt..999*2.*xmax.or.zcur.lt.1.d-4)then
            tflag=.TRUE.
         end if
      end if
   
   end subroutine inf_bound

!*****************************************************

   subroutine fresnel(sflag,tflag,iseed,weight,xcur,ycur &
   ,xmax,ymax,zmax,delta,xcell,ycell,zcell)
        
   use constants, only : pi,cbinsnum
   use photon_vars, only : nxp,nyp,nzp,cost,sint,cosp,sinp
   use opt_prop, only : n1, n2
   use iarray, only : trans

   implicit none
        

   real n,tir,cost2,f1,f2,xcur,ycur,xmax,ymax,zmax
   real ran2,costt,crit,ran,weight,delta
   integer iseed,ix,iy,xcell,ycell,zcell
   logical sflag,tflag
   ...
   tir=(n1-n2)**2/(n2+n1)**2
!         print*,'straight',acos(costt)*180./pi,tir,zcell
   elseif(acos(costt).gt.crit)then!total internal reflection
   tir=1.0
!               print*,'tir',acos(costt)*180./pi,tir,crit*180./pi,zcell
...
!               print*,'other',acos(costt)*180./pi,tir
   ...
   if(tir.lt.0..or.tir.gt.1..or.isnan(tir))print*,tir
   ran=ran2(iseed)
   if(ran.lt.tir)then
   !           photon reflected
      cost=-cost
