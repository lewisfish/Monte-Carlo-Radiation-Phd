program error
   ...
   real :: c1, c2, k1, k2, a, b
   integer :: io, i, nzg
   
   open(89,file='fit_params.txt', status = 'old', IOSTAT = io)
      read(89,*) c1, c2, k1, k2
   close(89)
   
   call readfile_array2D("jmean430.dat", array, 0, 2)
   nzg=size(array) / 2
   open(45, file = "error.dat")
   do i=1, nzg
!      write(45,*) array(i,1) ,abs(1.-(array(i,2)/((6.27*exp(-array(i,1)/.261))-(1.18*exp((-14.4*array(i,1))/.261)))))
      a=(c1*exp(-k1*array(i,1)/.261))-(c2*exp((-k2*array(i,1))/.261))
      b=(6.27*exp(-array(i,1)/.261))-(1.18*exp((-14.4*array(i,1))/.261))
      write(45,*) array(i,1) ,abs(1.- (a/b))
   end do
